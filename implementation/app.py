"""
SRE Academy Training Application - Exercise 11
Fully instrumented Flask application with:
- OpenTelemetry tracing (distributed traces)
- Prometheus metrics (Counter, Histogram, Gauge)
- OTLP log exporting
- Custom business logic with error simulation
"""
...
logger.debug('Application started - Debug level')
...
# Histogram: tracks distribution of values (latency percentiles)
REQUEST_LATENCY = Histogram('http_request_duration_seconds', 'HTTP Request Latency', ['method', 'endpoint'])

# Gauge: value that can go up and down (current state)
...
def zoo():
    """Simulates a database operation with random latency"""
...
def goo():
    """Business logic function with error simulation (every 5th call fails)"""
    ...
    with tracer.start_as_current_span("goo") as span:
        try:
            # Simulate errors on every 5th call for SRE testing
...
def foo():
    """Orchestration function that calls goo"""
...
@app.get('/store')
def get_stores():
    """Main endpoint that exposes store data and triggers traced operations"""
    ...
    REQUEST_LATENCY.labels(method='GET', endpoint='/store').observe(latency)
...
def metrics():
    ...
    Exposes all metrics collected by prometheus_client in Prometheus format
    Scraped by Prometheus server via kubernetes_sd_configs
    """
